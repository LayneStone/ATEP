...
export class SuiContractAdapter {
  ...
  async createTask(params: {
    ...
    let nostrPublishedTo: string[] | undefined;

    // 如果设置了 Nostr 发布器且提供了参数，广播到 Nostr
    if (success && this.nostrPublisher && params.nostrParams) {
      try {
        const nostrResult = await this.nostrPublisher.publishTaskAnnouncement(
          Buffer.from(params.taskId).toString('hex'),
          {
            ...params.nostrParams,
            arbitratorSuiAddress: params.arbitratorSuiAddress,
            amount: params.rewardAmount.toString(),
            txHash: result.digest,
            suiObjectId: taskObjectId,
          }
        );
        nostrEventId = nostrResult.eventId;
        nostrPublishedTo = nostrResult.publishedTo;
      } catch (nostrError) {
        console.warn('Nostr broadcast failed:', nostrError);
      }
    }
